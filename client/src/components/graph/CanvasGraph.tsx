...
const CanvasGraph: FC = () => {
  ...
  useEffect(() => {
    ...
    canvas.onwheel = (ev) => {
      cam.zoom =
        Math.abs(grapher.cam_zoom() + ev.deltaY * ZOOM_SENSITIVITY) + MIN_ZOOM;
    };

    const lerp = (from: number, to: number, a: number) =>
      from + (to - from) * a;
  ...
  const handleReload = () => {
    ...
    const equations = graph.equations.map((equation) => [
      equation.disabled,
      equation.equation,
    ]);
  ...
  return (
    <>
      <canvas
        ...
        ref={canvasRef}
      />
      <button id="graph-reload" onClick={handleReload}>
        Reload
      </button>
    </>
  );
