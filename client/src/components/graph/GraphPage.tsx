...
const EquationDropdown: FC<{ z: number }> = ({ z }) => {
  ...
  return (
    <span className="dropdown">
      ...
      <img src="icons/trash.svg" className="delete" onClick={handleDelete} />
    </span>
  );
};
...
const EquationPrompt: FC<EquationPromptProps> = ({ equation }) => {
  ...
  const onInputKey: KeyboardEventHandler = (ev) => {
    ...
    if (RESTRICTED_CHARACTERS.includes(ev.key)) ev.preventDefault();
  };
...
const EquationsPanel: FC = () => {
  ...
  const equations = graph?.equations.map((eq) => (
    <EquationPrompt equation={eq} key={eq.zIndex} />
  ));
  ...
  return (
    <>
      ...
      <div className={"equations-panel" + (visible ? "" : " hide")}>
        <ul className={"equations"}>{equations}</ul>
...
export const GraphDataContext = createContext<
  [GraphData | null, Dispatch<SetStateAction<null | GraphData>>]
>([null, () => {}]);
    ...
    <CrateContext.Provider value={crate}>
      <GraphDataContext.Provider value={[graph, setGraph]}>
        <div className="graph-page">
          ...
          <GraphSidePanel />
          <EquationsPanel />
          <h1 id="graph-name" onClick={() => setGraph(null)}>
            {graph.name}
          </h1>
        </div>
